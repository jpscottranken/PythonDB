...
def vowelOR(conn):
  ...
  try:
    ...
    vowelORquery = """
                  ...
                  OR   (lower(CustomerLastName)    LIKE '%a'
                  ...
                  ORDER BY CustomerLastName ASC,
                           CustomerFirstName            
                    """
    c.execute(vowelORquery)   
    print("\nQ14. Those employees whose first name starts with a vowel OR whose last name ends with a vowel  (unformatted):")
    print(c.fetchall())
    ...
    print(f"\nQ14. Problem trying to display database records: {e}")
    return None