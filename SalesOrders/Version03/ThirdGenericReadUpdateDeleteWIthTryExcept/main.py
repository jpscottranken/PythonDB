...
"""
	Here is the SQLite3 Exception Hierarchy:
	========================================
	sqlite3.Error
	...
	In Python's sqlite3 module, several exceptions are defined 
	to help developers handle errors that may occur while 
	interacting with SQLite databases.
	
	These exceptions allow for better error handling and debugging.
	Here's an overview of the main exceptions and what they mean:

    1. 	sqlite3.Error
		Base class for all exceptions raised by the sqlite3 module.
		It is the parent class for all other exceptions related to 
		SQLite operations.

    2.	sqlite3.DatabaseError
		Subclass of sqlite3.Error.
		Raised for errors related to the database itself (such as 
		corruption or problems in accessing the database file).

    3.	sqlite3.IntegrityError
	Subclass of sqlite3.DatabaseError.
	Raised when a database constraint (like a primary key, unique, 
	or foreign key constraint) is violated.

    4.	sqlite3.ProgrammingError
	Subclass of sqlite3.DatabaseError.
	Raised for programming errors, such as incorrect SQL syntax or 
	misuse of the SQLite API.

    5.	sqlite3.OperationalError
	Subclass of sqlite3.DatabaseError.
	Raised for operational errors, which include:
		Database locking errors (e.g. DB locked by another process).
		Disk I/O errors.

    6.	sqlite3.DataError
	Subclass of sqlite3.DatabaseError.
	Raised for problem with data itself (e.g., incorrect data type).

    7.	sqlite3.InternalError
	Subclass of sqlite3.DatabaseError.
	Raised when SQLite encounters an internal error, indicating 
	something went wrong inside the SQLite engine itself.

    8.	sqlite3.NotSupportedError
	Subclass of sqlite3.DatabaseError.
	Raised when an unsupported SQLite operation is attempted.

    9.	sqlite3.InterfaceError
	Subclass of sqlite3.Error.
	Raised when there is an error related to the database 
	interface (typically indicating a misuse of the Python sqlite3 
	interface, rather than a database-specific issue).

    10.	sqlite3.Warning
	This is NOT an error but a subclass of Python's built-in 
	Warning. It's used to issue warnings related to the SQLite module.
	
	Explanation:
	============
	1.	Create Operation (createProduct):
		Inserts a new record. Errors such as sqlite3.IntegrityError 
		or sqlite3.OperationalError are caught.

	2.	Read Operation (readProducts):
		Fetches all records from that table from the database.
        Handles possible database operational errors.

	3.	Update Operation (updateProduct): 
		Updates a record based on the given ID and allows 
		selective field updates (e.g., name, price, quantity).
        Errors e.g., sqlite3.IntegrityError or 
        sqlite3.OperationalError are caught.

	4.	Delete Operation (deleteProduct): 
...
def handleCRUD(conn, cursor, tableName):
  while True:
    ...
    if (operation == "1"):      # Create
      ...
      if (tableName == "Categories"):
        updateCategory(conn, cursor)
      elif (tableName == "Customers"):
        updateCustomer(conn, cursor)
      elif (tableName == "Employees"):
        updateEmployee(conn, cursor)
      elif (tableName == "OrderDetails"):
        updateOrderDetail(conn, cursor)
      elif (tableName == "Orders"):
        updateOrder(conn, cursor)
      elif (tableName == "Products"):
        updateProduct(conn, cursor)
      elif (tableName == "ProductVendors"):
        updateProductVendor(conn, cursor)
      elif (tableName == "Vendors"):
        updateVendor(conn, cursor)
    elif (operation == "4"):      # Delete
      if (tableName == "Categories"):
        deleteCategory(conn, cursor)
      elif (tableName == "Customers"):
        deleteCustomer(conn, cursor)
      elif (tableName == "Employees"):
        deleteEmployee(conn, cursor)
      elif (tableName == "OrderDetails"):
        deleteOrderDetail(conn, cursor)
      elif (tableName == "Orders"):
        deleteOrder(conn, cursor)
      elif (tableName == "Products"):
        deleteProduct(conn, cursor)
      elif (tableName == "ProductVendors"):
        deleteProductVendor(conn, cursor)
      elif (tableName == "Vendors"):
        deleteVendor(conn, cursor)
